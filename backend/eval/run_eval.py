...
from app.utils.prompt_loader import get_artist_identification_prompt_v2
...
ALL_CONFIGS: list[EvalConfig] = [
    # Baselines — always pinned to the newest production models
...
async def _run_model(image_bytes: bytes, prompt: str, model_key: str, model_id: Optional[str] = None) -> dict:
    ...
    resp = await service.identify_artist(image_bytes)
    return _parse_response(resp)


async def _run_gpt_web_search(image_bytes: bytes, prompt: str) -> dict:
    """
    OpenAI Responses API with web_search_preview tool.
    Uses gpt-4o-search-preview which supports both image input and web search.
    """
    b64 = base64.b64encode(image_bytes).decode()
    payload = {
...
async def _run_gemini_web_search(image_bytes: bytes, prompt: str) -> dict:
    ...
    try:
        if config.model is None:
            ...
            if config.web_search:
                if config.model == "gpt":
                    predicted = await _run_gpt_web_search(image_bytes, prompt)
                elif config.model == "gemini":
                    predicted = await _run_gemini_web_search(image_bytes, prompt)
                else:
...
async def main():
    ...
    base_prompt = get_artist_identification_prompt_v2("default")
