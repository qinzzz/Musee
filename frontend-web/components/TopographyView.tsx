  ...
  return (
    <div className="fixed inset-0 z-[60] bg-[#fdfdfd] overflow-hidden cursor-grab active:cursor-grabbing select-none animate-in fade-in duration-700"
      ...
      <div className="absolute top-10 left-10 z-10 space-y-2 pointer-events-none">
        <h2 className="text-xl font-extralight tracking-[0.4em] uppercase text-neutral-900">Semantic Topography</h2>
      ...
      <div className="absolute left-10 bottom-10 z-10 max-w-[450px] flex flex-wrap gap-2 pointer-events-auto">
        {tagNodes.map(node => {
          ...
          return (
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-[9px] tracking-widest uppercase transition-all border ${
                node.isExternal 
                  ? 'border-amber-200 text-amber-700 bg-amber-50/50 hover:bg-amber-100' 
                  : (selectedTag === node.tag || isRelatedToArtwork
                    ? 'bg-neutral-900 text-white border-neutral-900 scale-110 shadow-lg' 
                    : 'bg-white/80 backdrop-blur text-neutral-400 border-neutral-100 hover:border-neutral-300')
              } ${isRelatedToArtwork ? 'ring-2 ring-offset-2 ring-neutral-200' : ''}`}
            >
              {node.isExternal && <span className="mr-1.5">◈</span>}
              {node.tag}
            </button>
          );
        })}
        {(selectedTag || selectedArtworkId) && (
          <button onClick={() => { setSelectedTag(null); setSelectedArtworkId(null); }} className="text-[9px] text-neutral-300 uppercase tracking-widest hover:text-neutral-900 p-2">Reset View</button>
        )}
      </div>
        ...
        {tagNodes.map(node => {
          ...
          return (
            ...
            >
              ...
              {/* Tag Label */}
              <div className={`px-3 py-1.5 bg-white/95 backdrop-blur rounded-full border shadow-sm transition-all ${
                node.isExternal 
