...
const EmptyWall: React.FC<Props> = ({ isVisitMode }) => {
  return (
    <div className={`w-screen h-full flex flex-col items-center justify-center shrink-0 ${isVisitMode ? 'bg-transparent' : 'bg-[#fdfdfd]'}`}>
      {/* Light Column */}
      ...
      <div className="relative z-10 flex flex-col items-center max-w-md text-center px-12 animate-in fade-in zoom-in-95 duration-1000">
        <div className="mb-12">
          <svg width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg" className={`opacity-10 ${isVisitMode ? 'stroke-white' : 'stroke-black'}`}>
        ...
        <p className={`text-[13px] font-serif italic leading-relaxed opacity-40 ${isVisitMode ? 'text-neutral-300' : 'text-neutral-400'}`}>
